...
export const schoolsRouter = t.router({
  ...
  getByQuery: t.procedure.input(getSchoolsByQuerySchema).query(({ input }) => {
    return getSchoolsByQuery(input);
  }),

  // GET /by-domain -> getByEmailDomain procedure
  getByEmailDomain: t.procedure.input(z.object({ domain: z.string() })).query(({ input }) => {
    return getSchoolByEmailDomain(input.domain);
  })
});
